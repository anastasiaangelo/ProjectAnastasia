...
for res in num_res:
    for rot in num_rot:
        ...
        def generate_linear_coupling_map(n_qubits):
        ...
        trivial_layout = Layout({qubit: i for i, qubit in enumerate(ansatz.qubits)})  

        # Transpile hamiltonian and initial state
        full_circuit_isa = transpile(ansatz, backend=None, initial_layout=trivial_layout,
                                    basis_gates=['cx', 'rz', 'x', 'sx'], coupling_map=linear_coupling_map,
        ...
        file_exists = os.path.isfile(file_path_depth)
        ...
        with open(file_path_depth, mode="a", newline="") as file:
            ...
            if not file_exists:
                writer.writerow(["Size", "Total Gates", "Depth", "CNOTs"])
            
            # Append the new result
            writer.writerow([num_qubits, total_gates, depth, CNOTs])
        ...
        trivial_layout = Layout({qubit: i for i, qubit in enumerate(ansatz_baseline.qubits)})  

        full_circuit_bl_isa = transpile(ansatz_baseline, backend=None, initial_layout=trivial_layout,
                                    basis_gates=['cx', 'rz', 'x', 'sx'], coupling_map=linear_coupling_map,
                                    optimization_level=optimization_level, layout_method=layout_method, routing_method=routing_method)
        ...
        full_circuit_pen_isa = transpile(ansatz_penalty, backend=None, initial_layout=trivial_layout,
                                    basis_gates=['cx', 'rz', 'x', 'sx'], coupling_map=linear_coupling_map,
                                    optimization_level=optimization_level, layout_method=layout_method, routing_method=routing_method)
