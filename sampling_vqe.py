...
class SamplingVQE(VariationalAlgorithm, SamplingMinimumEigensolver):
    ...
    def _get_evaluate_energy(
        ...
        def evaluate_energy(parameters: np.ndarray) -> np.ndarray | float:
            ...
            # This line runs the circuit with the current parameters and captures the resulting distributions
            result = self.sampler.run([ansatz] * len(parameters), parameters).result()
            # Added quasi distributions here to have access to intermediate bitstrings and energy values
            quasi_dists = result.quasi_dists
    
            self.all_quasi_distributions.append(quasi_dists[0])

            # Modified callback function
            if self.callback is not None:
                self.callback(quasi_dists, parameters, values)
